...
class API
{
    ...
    public function getSpace()
    {
        ...
        var_dump($response);
        file_put_contents('./space.txt', json_encode($response));
        $files = [];
    ...
    private function auth()
    {
        ...
        $expectedTitle = sprintf('Входящие - Почта Mail.Ru', $this->login);
        $authResponse = $this->http->post(
            static::AUTH_DOMAIN . '/cgi-bin/auth?lang=ru_RU&from=authpopup',
            [
                'form_params' => [
        ...
        try {
            ...
            //file_put_contents('test.html',$authResponse->getBody());
